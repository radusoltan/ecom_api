...
final readonly class WarehouseCode implements \Stringable
{
    ...
    private function validate(): void
    {
        ...
        if ($length < self::MIN_LENGTH || $length > self::MAX_LENGTH) {
            throw new \InvalidArgumentException(sprintf('Warehouse code must be between %d and %d characters long', self::MIN_LENGTH, self::MAX_LENGTH));
        }

        if (!preg_match(self::PATTERN, $this->value)) {
            throw new \InvalidArgumentException('Warehouse code must contain only uppercase alphanumeric characters (A-Z, 0-9) and hyphens');
        }
    }
