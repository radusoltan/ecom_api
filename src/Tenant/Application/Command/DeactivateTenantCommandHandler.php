...
#[AsMessageHandler]
final readonly class DeactivateTenantCommandHandler
{
    ...
    {
        $this->profiler->start('tenant.deactivate');
        ...
        try {
            ...
            $metrics = $this->profiler->stop('tenant.deactivate');

            // Log if slow (> 200ms for write operations)
            if ($metrics['duration_ms'] > 200) {
                $this->logger->warning('Slow tenant deactivation detected', [
                    'tenant_id' => $command->tenantId,
                    'metrics' => $metrics,
                ]);
            }
        } catch (\Throwable $e) {
            $this->profiler->stop('tenant.deactivate');
