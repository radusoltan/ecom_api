...
#[AsMessageHandler]
final readonly class ReactivateTenantCommandHandler
{
    ...
    {
        $this->profiler->start('tenant.reactivate');
        ...
        try {
            ...
            $metrics = $this->profiler->stop('tenant.reactivate');

            // Log if slow (> 200ms for write operations)
            if ($metrics['duration_ms'] > 200) {
                $this->logger->warning('Slow tenant reactivation detected', [
                    'tenant_id' => $command->tenantId,
                    'metrics' => $metrics,
                ]);
            }
        } catch (\Throwable $e) {
            $this->profiler->stop('tenant.reactivate');
