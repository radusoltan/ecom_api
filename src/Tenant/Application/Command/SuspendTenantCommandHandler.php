...
#[AsMessageHandler]
final readonly class SuspendTenantCommandHandler
{
    ...
    {
        $this->profiler->start('tenant.suspend');
        ...
        try {
            ...
            $metrics = $this->profiler->stop('tenant.suspend');

            // Log if slow (> 200ms for write operations)
            if ($metrics['duration_ms'] > 200) {
                $this->logger->warning('Slow tenant suspension detected', [
