 ...
 * - Immutable identifier for payment transactions
 * - Used for tracking individual payment operations (auth, capture, refund, etc.)
 */
final readonly class TransactionId implements \Stringable
{
    private function __construct(private string $value)
    {
        ...
        if (!preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i', $value)) {
            throw new \InvalidArgumentException(sprintf('Invalid TransactionId format: "%s". Must be a valid UUID v4', $value));
        }
    }
