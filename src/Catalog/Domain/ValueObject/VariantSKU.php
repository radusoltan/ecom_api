...
final readonly class VariantSKU
{
    ...
    public static function generate(string $baseSku, array $valueCodes): self
    {
        ...
        if (!preg_match(self::BASE_PATTERN, $baseSku)) {
            throw new \InvalidArgumentException(
                sprintf('Invalid base SKU format: %s. Expected format: AAA-BBB-000000', $baseSku)
            );
        }
    ...
    private function validate(string $value): void
    {
        ...
        if (strlen($value) > self::MAX_LENGTH) {
            throw new \InvalidArgumentException(
                sprintf('Variant SKU cannot exceed %d characters', self::MAX_LENGTH)
            );
        }
        ...
        if (!preg_match(self::FULL_PATTERN, $value)) {
            throw new \InvalidArgumentException(
                sprintf(
                    'Invalid variant SKU format: %s. Expected format: AAA-BBB-000000-suffix (e.g., "CLO-NIK-000123-red-xl")',
                    $value
                )
            );
        }
