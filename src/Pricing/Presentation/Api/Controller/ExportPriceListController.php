...
use App\Pricing\Application\Query\ExportPriceLists\ExportPriceListsQuery;
...
/**
 * Controller for exporting PriceLists to CSV/JSON.
 *
 * Endpoint: GET /api/v1/price-lists/export
 */
#[Route('/api/v1/price-lists', name: 'api_price_lists_')]
final class ExportPriceListController extends AbstractController
{
    ...
    public function export(Request $request): Response
    {
        try {
            ...
            // Create query
            $query = new ExportPriceListsQuery($filter, $format);
            ...
            if ($query->isCsvFormat()) {
                ...
                $filename = 'price-lists-export-' . date('Y-m-d-His') . '.csv';
            } else {
                $contentType = 'application/json';
                $filename = 'price-lists-export-' . date('Y-m-d-His') . '.json';
            }
    ...
    {
        try {
            $content = $this->exportService->getPriceListCsvTemplate();

            $response = new Response($content);
            $response->headers->set('Content-Type', 'text/csv');
            $response->headers->set('Content-Disposition', 'attachment; filename="price-list-import-template.csv"');
            $response->headers->set('Cache-Control', 'no-cache, no-store, must-revalidate');
