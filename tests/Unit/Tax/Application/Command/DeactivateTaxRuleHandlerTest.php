...
final class DeactivateTaxRuleHandlerTest extends TestCase
{
    private TaxRuleRepositoryInterface $repository;
    private DeactivateTaxRuleHandler $handler;
    ...
    protected function setUp(): void
    {
        ...
        $this->handler = new DeactivateTaxRuleHandler($this->repository);
    }
    ...
    #[Test]
    public function itDeactivatesAnActiveTaxRule(): void
    {
        ...
        ($this->handler)(new DeactivateTaxRule(id: $id, tenantId: $tenantId));

        self::assertFalse($taxRule->isActive());
    }
    ...
    public function itThrowsWhenTaxRuleBelongsToAnotherTenant(): void
    {
        ...
        $taxRule = $this->buildActiveTaxRule($id, $ownerTenant);
        $this->repository->method('findById')->willReturn($taxRule);

        $this->expectException(\DomainException::class);
        $this->expectExceptionMessage('does not belong to this tenant');

        ($this->handler)(new DeactivateTaxRule(id: $id, tenantId: $callerTenant));
    }
    ...
    #[Test]
    public function itThrowsWhenTaxRuleIsAlreadyInactive(): void
    {
        $tenantId = TenantId::fromString(self::TENANT_ID);
        $id = TaxRuleId::generate();
