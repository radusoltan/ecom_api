...
use App\Invoice\Application\Command\CancelInvoice\CancelInvoiceHandler;
...
#[CoversClass(CancelInvoiceHandler::class)]
final class CancelInvoiceHandlerTest extends TestCase
{
    private InvoiceRepositoryInterface $repository;
    private CancelInvoiceHandler $handler;

    protected function setUp(): void
    {
        $this->repository = $this->createMock(InvoiceRepositoryInterface::class);
        $this->handler = new CancelInvoiceHandler($this->repository, new NullLogger());
    }

    #[Test]
    public function handleCancelsInvoiceAndSaves(): void
    {
        $invoiceId = InvoiceId::generate();
        $invoice = $this->createIssuedInvoice($invoiceId);

        $command = new CancelInvoiceCommand($invoiceId->toString());
        ...
        $this->repository->expects(self::once())
            ...
            ->with(self::callback(function (Invoice $saved): bool {
                return $saved->status()->isCancelled();
            }));
    ...
    {
        $command = new CancelInvoiceCommand(InvoiceId::generate()->toString());
